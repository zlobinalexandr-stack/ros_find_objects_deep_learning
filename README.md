...
| `header.frame_id` | `output_frame`, по умолчанию `map` |
| `child_frame_id` | `object_frame_prefix` + ID, по умолчанию `object_7` |
| `transform.translation.{x,y,z}` | положение в `output_frame`; единицы совпадают с масштабированной depth-картой |
| `transform.rotation.{x,y,z,w}` | `(0, 0, 0, 1)`, то есть единичный quaternion |

До публикации узел преобразует через собственный TF2 listener полную точку `(x, y, z)` из
оптического frame, указанного в `CameraInfo` (с резервным frame depth-сообщения), в
`output_frame`. Поэтому клиент может использовать сырые `translation.x/y` опубликованного TF для
отрисовки на карте. Оптическая глубина `z`, например 4.3 м, участвует в преобразовании и не
отбрасывается. Если преобразование в момент depth-кадра недоступно, обнаружение не публикуется под
ложным parent frame: узел выводит диагностический WARN с исходным и целевым frame.

При стандартном `output_frame=map` опубликованное сообщение уже содержит преобразованные
координаты `map -> object_<id>`. Для ручной проверки, пока детектор видит объект:

```bash
rosrun tf tf_echo map object_1
```

Перед публикацией координаты каждого ID независимо сглаживаются экспоненциальным
скользящим средним уже в `output_frame`. Параметр `position_smoothing_factor` задаёт долю
нового измерения: значение по умолчанию `0.25` подавляет дребезг, меньшее значение даёт
более плавное, но более запаздывающее движение, а `1.0` отключает сглаживание. После
перерыва дольше `position_filter_reset_time` (по умолчанию `1.0` с) фильтр начинает с
новой позиции, чтобы старое обнаружение не притягивало маркер. Оба параметра доступны
как аргументы launch-файла, например:

```bash
roslaunch find_object_3d_web find_object_3d_web.launch \
  position_smoothing_factor:=0.15 position_filter_reset_time:=1.0
```
...
`object_1` и отрисовывать возвращённые TFClient `translation.x/y`. При стандартной конфигурации те
же координаты находятся непосредственно в `translation.x/y` сообщения `/tf`.
...
`CameraInfo`, устаревшая depth-метка, невалидная глубина около центра объекта или недоступное
преобразование из оптического frame камеры в `output_frame`.
...
и префикс TF. Параметр `output_frame` выбирает parent frame результата и по умолчанию
равен `map`; его также можно передать одноимённым launch-аргументом. Узел намеренно
не использует `/depthnet/colormap`: цветовая карта
