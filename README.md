...
пустым `header.frame_id`. Нативный генератор на C++ использует API
`find_object_2d` для создания совместимого `session.bin`; менеджер перезапускает
детектор с параметром `session_path` после добавления или удаления объекта. Все
...
запрашивает пересборку session и перезапуск детектора. Пустой `data` отмечается
...
После добавления или удаления список публикуется заново, `session.bin`
пересобирается нативной библиотекой `find_object_2d`, а детектор автоматически
перезапускается. По умолчанию исходные файлы хранятся в
`~/.ros/find_object_3d_web/objects`, session — в
`~/.ros/find_object_3d_web/session.bin`. Пути изменяются параметрами
`storage_directory` и `session_path`. Удаление отсутствующего или неположительного
ID отклоняется.
...
Пути хранилища и session можно изменить отдельно:

```bash
roslaunch find_object_3d_web find_object_3d_web.launch \
  storage_directory:=/data/find_objects \
  session_path:=/data/find_objects.bin
```

При старте `find_object_2d_session_manager` сначала создаёт session из всех
`<id>.image` в хранилище и только затем запускает `/find_object_2d`. Это исключает
гонку, при которой детектор стартует раньше восстановления эталонов.
